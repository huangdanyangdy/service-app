<template>
	<view >
		<uni-nav-bar left-icon="back" left-text="返回" right-text="刷新" title="全部工单" @clickLeft="doBack"
		...
		<view class="search_wrap"  v-if="list2[0]">
			<view class="search_txt p-2 pbt-3">
				搜索结果
			</view>
			<view class="search_list" @click="listDetail(item.orderId)" v-for="item in list2" :key="item.index">
				
				<view class="space-between pb-2"  >
					<view class="serve_sort">保洁类<text class="serve_price">￥30.00</text></view>
					<view class="sever_status" v-if="item.orderStatus===0">待处理</view>
					<view class="sever_status" v-if="item.orderStatus===1">处理中</view>
					<view class="sever_status" v-if="item.orderStatus===2">完成</view>
				</view>
				...
				<view class="space-between p-2"  v-if="item.orderStatus===1||item.orderStatus===2">
					<view class="header_img" v-if="item.staffList.imgUrl"><image :src="item.staffList.imgUrl"></image></view>
					<view class="header_img" v-else><image src="../../static/logo.png"></image></view>
					<view class="search_item">
		...
		<view>
			<!-- <view></view> -->
			<view class="search_list"  @click="listDetail(item.orderId)" v-for="item in all_List" :key="item.index" >
				<view class="space-between pb-2">
					<view class="serve_sort">保洁类<text class="serve_price">￥30.00</text></view>
					<view class="sever_status" v-if="item.orderStatus===0">待处理</view>
					<view class="sever_status" v-if="item.orderStatus===1">处理中</view>
...
<script>
	...
	import {getAllListMsg, getDealingStaffMsg} from "@/api/data.js"
	export default {
		...
		methods: {
			listDetail(e){
				console.log(e,"e")
				uni.navigateTo({
					url:"../listDetaile/listDetail?orderId="+e
				})
			},
			doInput(e){
				console.log(e,"父组件输入")
